...
export default function CreateGoal() {
        ...
        const unsubscribe = auth.onAuthStateChanged(async (user) => {
            if (!user) {
                ...
                try {
                    ...
                    setActiveGoalsCount(snapshot.docs.length);
                } catch (error) {
    ...
    // Fetch user's active goals count
    // const fetchActiveGoalsCount = async (userId) => {
    //     try {
    //         const goalsQuery = query(
    //             collection(db, 'goals'),
    //             where('userId', '==', userId)
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Success! Navigate to the newly created goal
            ...
            if (error.code === 'permission-denied') {
                setError('Permission denied. Please check if you are logged in with the correct account.');
